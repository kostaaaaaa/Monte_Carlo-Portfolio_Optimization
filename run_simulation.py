...
# Fixed parameters
risk_free_rate = 0.02 # fredapi can be used if one has a key
bounds = [(0, 0.4) for _ in tickers]
initial_weights = np.array([1 / len(tickers)] * len(tickers))
years_range = [1, 3]  # Range of years for each simulation
...
# If no valid tickers, exit
if adj_close_df.empty:
    print("No valid tickers were provided. Exiting.")
    exit()

# Calculate log returns once and covariance matrix
...
for _ in range(n_simulations):
    # Adjust the time range by subsetting data to a random number of years
